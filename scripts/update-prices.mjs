...
query GetFloorPrice($slugs: [String!]!, $rarity: Rarity!) {
  players(slugs: $slugs) {
    slug
    lowestPriceAnyCard(rarity: $rarity, inSeason: true) {
      liveSingleSaleOffer {
        receiverSide {
          amounts {
            referenceCurrency
...
function extractEuroPrice(amounts) {
  if (!amounts) return null;
  const { referenceCurrency, eurCents, gbpCents, usdCents } = amounts;
  let cents = null;
  if (referenceCurrency === 'EUR') cents = eurCents;
  else if (referenceCurrency === 'GBP') cents = gbpCents;
  else if (referenceCurrency === 'USD') cents = usdCents;
